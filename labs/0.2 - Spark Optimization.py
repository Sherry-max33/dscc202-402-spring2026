...
simple_dedup_df = dups_df.dropDuplicates(
    ["customerID", "first_name", "last_name", "email_address"])  # List of column names
...
usa_franchises_df = spark.table("samples.bakehouse.sales_franchises").filter(col("country") == "US") # Load franchises and filter country == "US"

# Step 3: Join transactions with clean customers, then with US franchises
