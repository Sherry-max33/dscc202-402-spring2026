...
franchise_metrics_df = (transactions_df
    ...
    .agg(sum("totalPrice").alias("total_revenue"),
         ...
         round(avg("totalPrice"), 2).alias("avg_transaction_price"))
    .orderBy(desc("total_revenue"))
    .limit(5)
)

average_customer_spend_df = (transactions_df
        .groupBy("franchiseID", "customerID")
        .agg(sum("totalPrice").alias("customer_total_spend"))
        .groupBy("franchiseID")
        .agg(round(avg("customer_total_spend"), 2).alias("avg_customer_spend"))
        .orderBy(desc("avg_customer_spend"))
)

popular_payment_category_df = (transactions_df
     .withColumn("payment_category", categorize_payment_udf(col("paymentMethod")))
     .groupBy("payment_category")
     .agg(count("*").alias("payment_usage_count"))
     .orderBy(desc("payment_usage_count"))
     .limit(1)
)

day_of_week_with_revenue_df = (transactions_df
       .withColumn("day_name", date_format(col("dateTime"), "EEEE"))
       .groupBy("day_name") 
       .agg(sum("totalPrice").alias("total_revenue"))
       .orderBy(desc("total_revenue"))
...
executive_franchise_summary_df = franchise_metrics_df.join(
     average_customer_spend_df,
     "franchiseID", 
     "left")

display(executive_franchise_summary_df)
display(average_customer_spend_df)
display(popular_payment_category_df)
display(day_of_week_with_revenue_df)
